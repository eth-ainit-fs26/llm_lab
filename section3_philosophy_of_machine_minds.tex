...
\setbeamercolor{block title}{fg=white, bg=mBlack}
...
\begin{tikzpicture}[
    box/.style={draw, rounded corners=4pt, minimum width=3.8cm,
                ...
                line width=1pt},
    eq/.style={font=\Huge\bfseries, text=accent}
  ]
  \node[box] (pred) {Prediction};
  \node[eq, right=1.2cm of pred] (e1) {=};
  \node[box, right=1.2cm of e1] (comp) {Compression};
  \node[eq, right=1.2cm of comp] (e2) {=};
  \node[box, right=1.2cm of e2] (intel) {Intelligence};

  \node[below=0.5cm of pred, font=\small\color{muted}] {Shannon, 1948};
  \node[below=0.5cm of comp, font=\small\color{muted}] {Kolmogorov, 1963};
  \node[below=0.5cm of intel, font=\small\color{muted}] {Hutter, 2005};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
    bar/.style={fill=mBlack, minimum width=1cm},
...
\begin{tikzpicture}[scale=0.85]
  ...
  \node[font=\Large, anchor=east] at (-0.5, 3.5) {你好};
  \draw[-{Stealth}, thick, accent] (0.3, 3.5) -- (1.5, 3.0);

  % Output slot
  \draw[thick, accent] (7.7, 3.5) -- (8.3, 3.5);
  \node[font=\Large, anchor=west] at (8.5, 3.5) {你好};
...
\begin{tikzpicture}[
  ...
  \node[evidence] (e1) at (0, 3.5)
    {Builds world models from sequences alone};
  \node[ref, right=0.3cm of e1] {Othello-GPT, 2023};

  \node[evidence] (e2) at (0, 1.8)
    {Compresses domains it has never seen};
  \node[ref, right=0.3cm of e2] {D\'{e}letang et al., 2024};

  \node[evidence] (e3) at (0, 0.1)
    {Develops spatial \& temporal coordinate systems};
  \node[ref, right=0.3cm of e3] {Gurnee \& Tegmark, 2024};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  \node[ref, right=0.3cm of e1] {\emph{Nature Human Behav.}, 2025};

  \node[evidence] (e2) at (0, 0.8)
    {Multimodality is neither necessary\\
     nor sufficient for grounding};
  \node[ref, right=0.3cm of e2] {``Vector Grounding Problem,'' 2023--25};
\end{tikzpicture}
\end{center}
