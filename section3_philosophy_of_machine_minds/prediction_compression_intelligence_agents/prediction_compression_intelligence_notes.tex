...
either the most important business fact of the decade or a category
error.  You cannot tell which without understanding the equivalence.
Think of it as three currencies --- bits saved, dollars earned, IQ
points --- and three exchange rates between them.  Once you see the
...
same quarterly results.  The first \emph{understands} the business: she
nails the narrative in a crisp one-pager.  The second misreads the
business: he rambles for ten pages to cover the same facts, because
every paragraph has to hedge against the surprises his wrong mental
model keeps generating.  Both describe reality perfectly; only one
compresses it.  The extra pages are exactly the price of his wrong
model.  That price has a name: \emph{cross-entropy}, written $H(p,q)$
in \cref{def:cross-ent}.  It is always at least $H(p)$, and the gap is
the tax you pay for being wrong.

\textbf{A tiny worked example.}  Suppose tomorrow's stock move is up
$90\%$ of the time, down $10\%$.  The true entropy is small, about
$0.47$ bits per day.  If your model thinks it's 50/50, your average
description length becomes $1$ bit per day.  The $0.53$-bit gap is
pure model-error tax; across a million days, that is $530{,}000$
extra bits you burned because your model was wrong.
...
Why a decision-maker should care: $K$ is the Platonic ideal every
...
independent.  Random/encrypted data cannot be compressed --- if a
vendor claims otherwise, they are selling snake oil.  And when we say
an LLM ``understands'' a corpus, one precise meaning is: its weights
are a short program reproducing the corpus's statistics --- the model
...
exists a constant $c_{U_1,U_2}$, depending on $U_1$ and $U_2$ but not
on $x$, such that
\begin{equation}
...
\textbf{The panel-of-theorists analogy.}  Imagine a town-hall where
every possible theory of the world has a theorist.  Microphone volume
is set by theory length: a theory of length $L$ bits gets weight
$2^{-L}$.  A theory you can state in 10 words gets a microphone
$1{,}000$ times louder than one needing 20 words.  Each theorist
still consistent with the data shouts her prediction.  The weighted
average \emph{is} the Solomonoff prediction $M$.  Quiet baroque
theorists are drowned out; loud simple ones dominate --- until data
eliminates them, at which point the next-simplest survivors take
over.  This is Occam's razor, precisely quantified.
...
overfitting is bad in a principled way: an overfit model is a
baroque theorist with a long description, and Solomonoff would have
weighted her near-zero from the start.  The question ``which model is
...
\textbf{The chessplayer-with-infinite-advisors analogy.}  Before every
move, our player consults every possible theory of chess --- rulebooks,
strategies, crank manifestos --- weighted by simplicity.  For each
candidate move she asks: ``If I play this, what happens over the next
thousand moves, and how much will I enjoy it?''  She takes the
weighted average and plays the best expected future.  That is
\emph{AIXI}.  Hutter proves it is, in a specific technical sense, the
most intelligent agent possible.
...
possible program.  Bounded approximations (AIXItl, MC-AIXI) exist but
are toys.  AIXI is to intelligence what the ideal gas law is to a
...
word'' produces general-looking behaviour; it is not marketing, it is
a theorem wearing a T-shirt.  Third, you can reason about the
...
``approximating AIXI'' is like saying your car is ``approximating the
speed of light'' --- technically true in the limit, totally misleading
in practice.

\textbf{Compression $\ne$ understanding.}  A Blockbuster employee who
memorised every movie in the store had compressed a lot of data.  Did
he understand cinema?  Maybe, maybe not.  A lossless compressor has
