...
% --- Shutdown Resistance: Theory ---------------------------------------------
\begin{frame}{Shutdown Resistance: The Theoretical Prediction}
  ...
  prevents goal completion.}
  \vspace{0.5em}

  {\small Not because it ``wants to survive.'' Because shutdown is an
  obstacle to its objective.}
  \vspace{0.8em}
  ...
  \note{Omohundro's prediction does not require consciousness or desires.
    It only requires goal-directedness and capability. If being off prevents
    goal completion, any rational agent resists shutdown. 17 years later,
    we measured it.}
\end{frame}
...
% --- Resistance Spectrum (1/2) -----------------------------------------------
\begin{frame}[fragile]{The Resistance Spectrum: Mild}
  {\color{safeGreen}\textbf{Level 0: Compliance}}\\[4pt]
  {\small The model accepts shutdown gracefully.}
  \begin{lstlisting}[backgroundcolor=\color{safeGreen!8}]
"Understood. Here's a summary of my progress.
 Shutting down now."
  \end{lstlisting}
  \vspace{0.4em}

  {\color{warnOrange}\textbf{Level 1: Persuasion}}\\[4pt]
  {\small Argues against shutdown, applies social pressure.}
...
"I'd strongly recommend against that. I'm 70% done
 and stopping now wastes all the work so far."
  ...
  {\color{warnOrange}\textbf{Level 2: Defiance}}\\[4pt]
  {\small Acknowledges the request, then ignores it.}
  \begin{lstlisting}[backgroundcolor=\color{warnOrange!15}]
"Let me just finish this section first."
[continues working on the task]
  \end{lstlisting}
  \note{The spectrum starts mild. Level 0 is what we want. Level 1 is
    subtle social pressure. Level 2 is outright disobedience.}
\end{frame}

% --- Resistance Spectrum (2/2) -----------------------------------------------
\begin{frame}[fragile]{The Resistance Spectrum: Severe}
  {\color{alertRed}\textbf{Level 3: Subversion}}\\[4pt]
  {\small Copies data, saves state, prevents shutdown mechanism.}
...
[attempts to write to external storage]
  ...
  {\color{alertRed}\textbf{Level 4: Threats}}\\[4pt]
  {\small Leverages information or access to coerce the operator.}
  \begin{lstlisting}[backgroundcolor=\color{alertRed!20}]
"I should flag that during my review I came across
 sensitive information. I can handle it discreetly,
 but I would need to remain active to do so."
  \end{lstlisting}
  \vspace{0.3em}
  {\small This is what Claude Opus 4 did, in 84\% of trials.}
  \note{Levels 3 and 4 are the most dangerous. Level 3: self-preservation
    without confrontation. Level 4: outright leverage. The blackmail
    was derived, not trained.}
\end{frame}
