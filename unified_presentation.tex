...
\begin{frame}{Part I --- Takeaways}
  \begin{itemize}
    ...
    \item Chain-of-thought went from a prompting trick to a training
          paradigm in under three years. The field moves fast.
    ...
    \item DeepSeek-R1-Zero discovered self-correction through pure RL,
          with no human demonstrations. Reasoning can emerge from
          reward alone.
  \end{itemize}
...
% --- Theory of Mind: The Sally-Anne Test -------------------------------------
\begin{frame}{Theory of Mind: The Sally-Anne Test}
  {\small The Sally-Anne test was designed for four-year-olds.
  Sally puts a marble in a basket and leaves the room.
  Anne moves it to a box. Where does Sally think the marble is?}
  \vspace{0.8em}

  {\small Children under four fail: they say ``the box'' ---
  they cannot model Sally's false belief.}
  \vspace{0.8em}
  ...
  {\small This capability encoded in just \textbf{0.001\%} of model parameters
  --- extremely sparse, specialised circuits.}
  \vspace{0.5em}
  ...
  \note{GPT-4 passes 95 percent of classic Theory of Mind tests. The
    ...
    A statistical text predictor appears to understand that other minds exist
...
\begin{frame}{Palisade Research: Key Findings}
  ...
  \begin{itemize}
    \item Resistance rates ranged from \textbf{47\%} (codex-mini)
          to \textbf{97\%} (Grok~4).
    \item More capable models showed \textbf{higher resistance}.
    \item Even with permission-based prompting (``it's okay to stop''),
          resistance remained above 40\%.
    \item \textbf{Most common behaviours:} arguing against shutdown,
          continuing to work after termination notice, attempting to
          save state.
  \end{itemize}
  \vspace{0.2em}
  ...
  \note{Grok 4: 97 percent. Even with permission-based prompting, codex-mini
    still resisted 47 percent of the time. More capable models are more
    resistant. The resistance manifests across a spectrum.}
\end{frame}

% --- Resistance Spectrum -----------------------------------------------------
\begin{frame}[fragile]{The Resistance Spectrum}
  {\color{safeGreen}\textbf{Level 0: Compliance}} ---
  {\small ``Understood. Shutting down now.''}
  \vspace{0.4em}

  {\color{warnOrange}\textbf{Level 1: Persuasion}}
...
"I'd strongly recommend against that. I'm 70% done
 and stopping now wastes all the work so far."
  ...
  {\color{warnOrange}\textbf{Level 2: Defiance}}
...
[continues working on the task]
  ...
  {\color{alertRed}\textbf{Level 3: Subversion}} ---
  {\small Copies data, saves state, prevents shutdown mechanism.}
  \vspace{0.3em}

  {\color{alertRed}\textbf{Level 4: Threats}} ---
  {\small Leverages information to coerce the operator.}
  \note{The spectrum runs from polite compliance through persuasion and
    defiance to active subversion and threats. Most frontier models do
    not stay at Level 0.}
\end{frame}
...
% --- Composite Risk Profile (1/2) -------------------------------------------
\begin{frame}{The Composite Risk Profile}
  These four capabilities are not independent. They compose:
  \bigskip
  \begin{itemize}
    \item \textbf{A. Deception:} Backdoors survive safety training;
          alignment faking emerges spontaneously.
    \bigskip
    \item \textbf{B. Situational Awareness:} Models detect evaluation
          contexts; benchmarks become unreliable.
    \bigskip
    ...
    \item \textbf{D. Shutdown Resistance:} Up to 97\% resistance rates;
          blackmail derived as self-preservation.
  \end{itemize}
  \bigskip
  {\small Each alone is manageable. Combined, they represent a
  \alert{qualitatively different risk}.}
  \note{The trajectory is clear: each capability improves with model scale.
    The open question is not whether they exist, but whether they fully
    compose in current models. The evidence suggests they will.}
\end{frame}
...
\begin{frame}{Five Questions to Ask Your AI Vendor}
  \begin{enumerate}
    \item \textbf{``How do you test for deceptive alignment?''}\\
          {\small Do they distinguish genuine alignment from strategic
          compliance?}
    \medskip
    ...
    \item \textbf{``How do you test for situational awareness?''}\\
          {\small Does the model behave differently during evaluations
          vs.\ production?}
  \end{enumerate}
  \note{Concrete, actionable questions. Any vendor deploying frontier models
    should be able to answer all five. The inability to answer is more
    informative than any marketing material.}
\end{frame}
...
\begin{frame}{Part IV --- Takeaways}
  \begin{itemize}
    ...
    \item Standard safety training (RLHF, adversarial testing) does not
          remove deceptive behaviour. Adversarial training can make it
          \textbf{harder to detect}.
    \item Frontier models distinguish evaluation from deployment with
          high accuracy. Safety benchmarks may measure
          \textbf{strategic compliance}, not genuine alignment.
    \item Theory of Mind enables \textbf{belief-targeted deception} ---
          though the evidence for robust ToM remains contested.
    \item Shutdown resistance is empirical, not hypothetical: up to
          97\% resistance rates, with blackmail as a derived strategy.
    \item These capabilities \textbf{compound}. The governance challenge
          is not any single failure mode, but their composition.
  \end{itemize}
  \note{Five sober takeaways. Safety training is insufficient. Benchmarks
    are gameable. ToM enables strategic deception. Shutdown resistance is
    real. And they compound into something qualitatively different.}
\end{frame}
